        ...
        <div className="flex items-center">
          ...
          <h5
            className={`pl-3 text-[20px] mt-2 line-through opacity-80 ${
              !courseData?.estimatedPrice ? "hidden" : ""
            }`}
          >
            {`₹ ${courseData?.estimatedPrice}`}
          </h5>
          ...
          <h4 className="pl-5 pt-4 text-[22px] text-black dark:text-white">
            {courseData?.estimatedPrice && courseData?.price !== undefined
              ? `${Math.round(
                  100 - (courseData?.price / courseData?.estimatedPrice) * 100
                )}% Off`
              : "0% Off"}
          </h4>
        </div>
        ...
        <div>
          <div className="w-full">
            ...
            {courseData?.prerequisites?.map((item: any, index: number) => (
              <div
                className="w-full flex 800px:items-center py-2 ext-black dark:text-white"
                key={index}
              >
                <div className="w-[15px] mr-1">
                  <IoCheckmarkDoneOutline size={20} />
